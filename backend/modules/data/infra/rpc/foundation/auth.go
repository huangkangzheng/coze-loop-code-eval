...
type checkPermissionParam struct {
	...
	withoutSPID     *bool // 评测集接口鉴权不回调SPI，直接将space_id和owner_id传递给权限接口
	OwnerID         *string
				...
				Objects: []*authentity.AuthEntity{
					{
						...
						SpaceID:     gcond.If(gptr.Indirect(param.withoutSPID), gptr.Of(strconv.FormatInt(param.ResourceSpaceID, 10)), nil),
						OwnerUserID: gcond.If(gptr.Indirect(param.withoutSPID), param.OwnerID, nil),
					},
				},
			})
		}
