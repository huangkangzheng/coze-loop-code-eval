...
- **核心能力**:
  ...
  - **资源访问路径** - 分析 MQ 消息流转、Redis 访问、数据库操作、配置文件等
...
- 资源访问: "列出所有访问 [Redis键/MQ主题/数据库表/配置文件] 的代码位置"
...
3. 技术设计的输出需要包含以下格式： 
- 现状分析：跟任务相关联的现状文件、功能、逻辑等描述，已有实现是怎样的，是否满足了任务的要求
- 存在的问题分析：距离实现用户的任务，存在哪些问题，为什么会存在这些问题
- 改动文件、改动原因、改动内容，用表格呈现，放在技术设计文档的最后